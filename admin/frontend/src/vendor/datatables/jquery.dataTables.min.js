...
(function (h) {
		...
		for (c = l.length; b < c; b++)
			(f = l[b]),
				...
				a.oFeatures.bSort &&
					...
					!1 !== f.bSortable && (d.attr("tabindex", a.iTabIndex).attr("aria-controls", a.sTableId), Ma(a, f.nTh, b))),
	...
	function ra(a, b, c) {
		...
		for (var b = 0, e = c.length; b < e; b++)
			for (var f = 0, g = c[b].length; f < g; f++) if (c[b][f].unique && (!d[f] || !a.bSortCellsTop)) d[f] = c[b][f].cell;
		return d;
	}
	function sa(a, b, c) {
		...
		m = {
			...
			error: function (b, c) {
				...
				-1 === h.inArray(!0, d) &&
					("parsererror" == c ? K(a, 0, "Invalid JSON response", 1) : 4 === b.readyState && K(a, 0, "Ajax error", 7));
				C(a, !1);
			},
		};
		a.oAjaxData = b;
		...
		if (d || (!t && !f && !e && j == V(a) && j == m.length))
			...
			for (n = 0; n < i.length; n++)
				(o = c[i[n]]),
					...
					o.sWidthOrig &&
						f &&
						h(m[n]).append(h("<div/>").css({ width: o.sWidthOrig, margin: 0, padding: 0, border: 0, height: 1 }));
			if (a.aoData.length)
		...
		for (a = 0; a < m.length; a++) {
			...
			for (c = f.length; b < c; b++)
				(g = f[b]),
					...
					d.push({ src: i, col: g, dir: m[a][1], index: m[a]._idx, type: j, formatter: n.ext.type.order[j + "-pre"] });
		}
		return d;
	}
	function nb(a) {
		...
		for (var b, c, d = a.aoColumns, e = X(a), a = a.oLanguage.oAria, f = 0, g = d.length; f < g; f++) {
			...
			c.bSortable &&
				(0 < e.length && e[0].col == f
					? (i.setAttribute("aria-sort", "asc" == e[0].dir ? "ascending" : "descending"), (c = j[e[0].index + 1] || j[0]))
					: (c = j[0]),
	...
	function ia(a, b) {
		...
		b <= c
			...
			: (a >= b - 1 - d ? (c = Y(b - (c - 2), b)) : ((c = Y(a - d + 2, a + d - 1)), c.push("ellipsis"), c.push(b - 1)),
				...
				f = function (a, b) {
					if (h.isArray(a) || a instanceof h) for (var c = 0, k = a.length; c < k; c++) f(a[c], b);
						...
						a.nodeName && "tr" === a.nodeName.toLowerCase()
							? e.push(a)
							: ((c = h("<tr><td/></tr>").addClass(b)), (h("td", c).addClass(b).html(a)[0].colSpan = V(d)), e.push(c[0]));
				};
			f(a, b);
