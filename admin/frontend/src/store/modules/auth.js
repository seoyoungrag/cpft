...
export const setUserTemp = ({ carrierSeq, userSeq, userLoginId, userNm, userEmail, token }) => ({
	type: SET_USER_TEMP,
	payload: {
		carrierSeq,
		userSeq,
...
const registerEpic = (action$, state$) => {
	return action$.pipe(
		...
		withLatestFrom(state$),
		mergeMap(([action, state]) => {
			const { userLoginId, userLoginPw, userNm, userEmail, rememberMe } = state.auth.form;
...
const loginEpic = (action$, state$) => {
	return action$.pipe(
		...
		mergeMap(([action, state]) => {
			...
			return ajax.post(`/v1/signin/`, { id: userLoginId, password: userLoginPw }).pipe(
				map((response) => {
