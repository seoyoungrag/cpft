...
function OrderManage(props) {
	...
	const _changeWorkGroup701 = (e) => {
		setState((prevState) => ({
			...prevState,
			changeWorkGroup701: $(e.target).val(),
		}));
	};
	const _changeWorkGroup702 = (e) => {
		setState((prevState) => ({
			...prevState,
			changeWorkGroup702: $(e.target).val(),
		}));
	};
	const _changeWorkGroup703 = (e) => {
		setState((prevState) => ({
			...prevState,
	...
	const _getCarrierOrders = async () => {
		...
		if (data.list.length > 0) {
			...
			setState((prevState) => ({
				...prevState,
				orderList701: orderList701,
				orderList702: orderList702,
				orderList703: orderList703,
				orderList,
				orderList,
			}));
		}
		...
		if (data2.data.data.workGroups) {
			setState((prevState) => ({
				...prevState,
				workGroups: data2.data.data.workGroups,
			}));
		}
	};
...
*/
	const onAddClick = (name, nickname) => {
		let updated = false;
			...
			<main>
										...
										<div className="nav-tabs-wrapper">
											...
											<ul className="nav nav-tabs" data-tabs="tabs">
												<li className="nav-item">
													<a className="nav-link d-flex align-items-center active" href="#ing" data-toggle="tab">
														<i className="fas fa-fw fa-hourglass-start"></i> <span className="sm-display-none">진행중</span>
														<div className="ripple-container"></div>
													</a>
												</li>
												<li className="nav-item">
													<a className="nav-link d-flex align-items-center" href="#complete" data-toggle="tab">
														<i className="fa fa-fw fa-hourglass-half"></i> <span className="sm-display-none">채용종료</span>
														<div className="ripple-container"></div>
													</a>
												</li>
												<li className="nav-item">
													<a className="nav-link d-flex align-items-center" href="#temp" data-toggle="tab">
														<i className="fa fa-fw fa-hourglass-end"></i> <span className="sm-display-none">임시저장</span>
														<div className="ripple-container"></div>
													</a>
												</li>
												<li className="nav-item">
													<a className="nav-link d-flex align-items-center" href="#all" data-toggle="tab">
														<i className="fa fa-fw fa-hourglass"></i> <span className="sm-display-none">전체</span>
														<div className="ripple-container"></div>
													</a>
												</li>
											</ul>
             ...
             >
              운송그룹
												...
												<select className="form-control col-12" id="orderRegisWorkGroup" onChange={_changeWorkGroup701}>
												...
												<tbody className="col-12">
													{orderList701.length > 0
														? orderList701.map((obj, index) => {
																return (
																	<OrderManageItem
																		getCarrierOrders={_getCarrierOrders}
																		obj={obj}
																		key={obj.orderSeq}
																		rcritTypeCodes={rcritTypeCodes}
																		tonTypeCodes={tonTypeCodes}
																		carTypeCodes={carTypeCodes}
																		changeWorkGroup={changeWorkGroup701}
																		tabType={"ing"}
																		display={changeWorkGroup701 == obj.workGroupNm || changeWorkGroup701 == "all" ? "table-row" : "none"}
																	/>
																);
														  })
														: null}
             ...
             >
              운송그룹
												...
												<select className="form-control col-12" id="orderRegisWorkGroup" onChange={_changeWorkGroup702}>
													<option value="all">전체보기</option>
													{workGroups.length > 0
														? workGroups.map((obj, index) => {
																return (
														...
														? orderList702.map((obj, index) => {
																return (
																	<OrderManageItem
																		...
																		carTypeCodes={carTypeCodes}
																		tabType={"complete"}
																		display={changeWorkGroup702 == obj.workGroupNm || changeWorkGroup702 == "all" ? "table-row" : "none"}
																	/>
																);
														  })
														: null}
             ...
             >
              운송그룹
												...
												<select className="form-control col-12" id="orderRegisWorkGroup" onChange={_changeWorkGroup703}>
													<option value="all">전체보기</option>
													{workGroups.length > 0
														? workGroups.map((obj, index) => {
																return (
														...
														? orderList703.map((obj, index) => {
																return (
																	<OrderManageItem
																		...
																		carTypeCodes={carTypeCodes}
																		tabType={"temp"}
																		display={changeWorkGroup703 == obj.workGroupNm || changeWorkGroup703 == "all" ? "table-row" : "none"}
																	/>
																);
														  })
														: null}
             ...
             */}
             ...
             >
              운송그룹
												...
												<select className="form-control col-12" id="orderRegisWorkGroup" onChange={_changeWorkGroup}>
													<option value="all">전체보기</option>
													{workGroups.length > 0
														? workGroups.map((obj, index) => {
																return (
														...
														? orderList.map((obj, index) => {
																return (
																	<OrderManageItem
																		...
																		carTypeCodes={carTypeCodes}
																		tabType={"all"}
																		display={changeWorkGroup == obj.workGroupNm || changeWorkGroup == "all" ? "table-row" : "none"}
																	/>
																);
														  })
														: null}
