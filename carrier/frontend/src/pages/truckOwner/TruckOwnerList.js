...
function TruckOwnerList(props) {
	...
	React.useEffect(() => {
		if (carTypeCodes.length > 0 && tonTypeCodes.length > 0 && workDayCodes.length > 0) {
			if (carTypeCodes && tonTypeCodes && workDayCodes && !$.fn.DataTable.isDataTable($("#truckOwnerListTbl"))) {
				console.log(carTypeCodes);
				console.log(tonTypeCodes);
				console.log(workDayCodes);
				const table = $("#truckOwnerListTbl").DataTable({
						...
						},
						{
							...
							createdCell: function (td, cellData, rowData, row, col) {
								...
								cellData.map((obj, index) => {
									...
									console.log(obj.order.workDays);
									console.log(workDayCodes);
									let workDay = obj.order.workDays
										.map((obj1, index) => {
											return workDayCodes.filter((obj2, index) => {
												if (obj2.code == obj1) {
													return true;
												}
												// })[0].codeValue;
				...
				<div className="page-header pb-10 page-header-dark bg-gradient-primary-to-secondary">
					<div className="container-fluid">
						<div className="page-header-content">
							...
							<div className="page-header-subtitle">전체 차주 리스트를 조회합니다.</div>
						</div>
					</div>
				</div>
...
export default React.memo(TruckOwnerList);
